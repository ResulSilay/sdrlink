...
void decodeCPR(struct mode_s_msg *a) {
...
void mode_s_decode(mode_s_t *self, struct mode_s_msg *mm, unsigned char *msg) {
    ...
    if (mm->msgtype == 17) {
        ...
        if (mm->metype >= 1 && mm->metype <= 4) {
            ...
            /*if (mm->fflag) {
                mm->odd_cprlat = mm->raw_latitude;
                mm->odd_cprlon = mm->raw_longitude;
                mm->odd_cprtime = mstime();
            } else {
                mm->even_cprlat = mm->raw_latitude;
                mm->even_cprlon = mm->raw_longitude;
                mm->even_cprtime = mstime();
            }

            if (llabs(mm->even_cprtime - mm->odd_cprtime) <= 10000) {
                decodeCPR(mm);
            }
            */
...
void mode_s_detect(mode_s_t *self, uint16_t *mag, uint32_t maglen, mode_s_callback_t cb) {
    ...
    for (j = 0; j < maglen - MODE_S_FULL_LEN*2; j++) {
        ...
        //std::cerr << good_message << std::endl;
        //std::cerr << use_correction << std::endl;
